    ...
    public Mono<Void> handleMessage(String projectId, String sessionId,
        ...
        return deserializeFromJson(payload, ClientMessage.class)
                .flatMap(message -> {
                    ...
                    if (type == null) {
                        log.warn("[PresenceService] No message type: sessionId={}", sessionId);
                        return Mono.empty();
                    }
                    ...
                    case CURSOR -> {
                        if (!(message instanceof CursorClientMessage cursorMessage)) {
                            log.warn("[PresenceService] Invalid message format: sessionId={}", sessionId);
                            yield Mono.empty();
                        }
                        CursorPosition cursor = cursorMessage.getCursor();
                        if (cursor == null) {
                            log.warn("[PresenceService] No cursor data: sessionId={}", sessionId);
                            yield Mono.empty();
                        }
                        yield updateCursor(projectId, sessionId, cursor);
                    }
                    default -> {
                        log.warn("[PresenceService] Unhandled message type: type={}, sessionId={}", type,
                                sessionId);
                        yield Mono.empty();
                    }
                    };
                ...
                .onErrorResume(e -> {
                    log.warn("[PresenceService] Message handling failed: {}", e.getMessage());
                    return Mono.empty();
                });
    }
    ...
    private <T> Mono<String> serializeToJson(T object) {
        return Mono.fromCallable(() -> objectMapper.writeValueAsString(object))
                .onErrorMap(JsonProcessingException.class,
                        e -> new RuntimeException("[PresenceService] failed to serialize JSON", e));
    }

    private <T> Mono<T> deserializeFromJson(String json, Class<T> clazz) {
        return Mono.fromCallable(() -> objectMapper.readValue(json, clazz))
                .onErrorMap(JsonProcessingException.class,
                        e -> new RuntimeException("[PresenceService] failed to deserialize JSON", e));
    }
