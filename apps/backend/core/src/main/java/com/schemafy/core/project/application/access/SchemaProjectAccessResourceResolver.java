...
class SchemaProjectAccessResourceResolver implements ProjectAccessResourceResolver {

  private final GetProjectIdByAccessResourcePort projectIdPort;

  @Override
  public Set<ProjectAccessResourceType> resourceTypes() {
    return Set.of(ProjectAccessResourceType.SCHEMA);
  }

  @Override
  public List<ProjectAccessAccessorRule> accessorRules() {
    return List.of(new ProjectAccessAccessorRule("schemaId", ProjectAccessResourceType.SCHEMA));
  }
  ...
  public Mono<ProjectAccessResourceRef> resolveParent(ProjectAccessResourceType type, String id) {
    return projectIdPort.findProjectId(type, id)
        .switchIfEmpty(Mono.error(
            new DomainException(SchemaErrorCode.NOT_FOUND, "Schema not found: " + id)))
