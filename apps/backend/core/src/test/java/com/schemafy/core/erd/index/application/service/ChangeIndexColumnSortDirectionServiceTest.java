...
class ChangeIndexColumnSortDirectionServiceTest {
  ...
  class ChangeIndexColumnSortDirection {
    ...
    @Test
    @DisplayName("lock 이후 이미 요청 정렬 방향이면 정의 조회 없이 변경 없이 성공한다")
    void returnsNoOpWhenLockedIndexColumnAlreadyHasRequestedSortDirection() {
      var command = IndexFixture.changeSortDirectionCommand("ic1", SortDirection.DESC);
      var initialIndexColumn = IndexFixture.indexColumn("ic1", "index1", "col1", 0, SortDirection.ASC);
      var lockedIndexColumn = IndexFixture.indexColumn("ic1", "index1", "col1", 0, SortDirection.DESC);
      var index = IndexFixture.index("index1", "table1", "idx_test", IndexType.BTREE);

      given(getIndexColumnByIdPort.findIndexColumnById("ic1"))
          .willReturn(Mono.just(initialIndexColumn), Mono.just(lockedIndexColumn));
      given(getIndexByIdPort.findIndexById("index1"))
          .willReturn(Mono.just(index));
