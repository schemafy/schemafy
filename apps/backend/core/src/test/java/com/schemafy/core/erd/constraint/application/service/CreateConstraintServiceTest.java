...
class CreateConstraintServiceTest {
  ...
  class CreateConstraint {
    ...
    void createsConstraintWithNameAtVendorLimit() {
      String name = "😀".repeat(64);
      var template = ConstraintFixture.createUniqueCommand();
      var command = new CreateConstraintCommand(
          template.tableId(),
          name,
          template.kind(),
          template.checkExpr(),
          template.defaultExpr(),
          template.columns());
      var table = createTable("table1", "schema1");
      var tableColumns = List.of(ColumnFixture.columnWithId(ConstraintFixture.DEFAULT_COLUMN_ID));

      given(getTableByIdPort.findTableById(command.tableId())).willReturn(Mono.just(table));
      given(constraintExistsPort.existsBySchemaIdAndName(table.schemaId(), name))
          .willReturn(Mono.just(false));
      given(getColumnsByTableIdPort.findColumnsByTableId(table.id()))
          .willReturn(Mono.just(tableColumns));
      given(getConstraintsByTableIdPort.findConstraintsByTableId(table.id()))
    ...
    void rejectsConstraintNameOverVendorLimit() {
      String name = "😀".repeat(65);
      var command = ConstraintFixture.createCommandWithName(name);
      var table = createTable("table1", "schema1");
    ...
    void truncatesAutoGeneratedNameForTwoDigitSuffix() {
      ...
      var tableColumns = List.of(ColumnFixture.columnWithId(ConstraintFixture.DEFAULT_COLUMN_ID));
      String expectedName = "pk_" + "t".repeat(58) + "_10";

      given(getTableByIdPort.findTableById(command.tableId())).willReturn(Mono.just(table));
      given(constraintExistsPort.existsBySchemaIdAndName(eq(table.schemaId()), anyString()))
