	...
	class ChangeColumnMeta {
		...
		class WithPartialUpdate {
			...
			void changesOnlyAutoIncrement() {
				var command = ColumnFixture.changeMetaCommand(
						PatchField.of(true), PatchField.absent(), PatchField.absent(), PatchField.absent());
			...
			void changesOnlyCharset() {
				var command = ColumnFixture.changeMetaCommand(
						PatchField.absent(), PatchField.of("utf8mb4"), PatchField.absent(), PatchField.absent());
				...
				then(changeColumnMetaPort).should()
						.changeColumnMeta(eq(command.columnId()), isNull(), eq("utf8mb4"), isNull(), isNull());
			}
			...
			void changesOnlyComment() {
				var command = ColumnFixture.changeMetaCommand(
						PatchField.absent(), PatchField.absent(), PatchField.absent(), PatchField.of("New comment"));
				var column = ColumnFixture.defaultColumn();
		...
		class WithExplicitNullClear {
			...
			void clearsCharsetToNull() {
				var command = ColumnFixture.changeMetaCommand(
						PatchField.absent(), PatchField.of(null), PatchField.absent(), PatchField.absent());
				...
				then(changeColumnMetaPort).should()
						.changeColumnMeta(eq(command.columnId()), isNull(), eq(""), isNull(), isNull());
			}
		...
		class WhenPkColumnCharsetChanged {
			...
			void propagatesCharsetCollationToFkColumns() {
				...
				then(changeColumnMetaPort).should()
						.changeColumnMeta(eq(pkColumn.id()), isNull(), eq("utf8mb4"),
		...
		class WhenMultiLevelCascade {
			...
			void propagatesCharsetCollationRecursivelyToDownstreamFkColumns() {
				...
				then(changeColumnMetaPort).should()
						.changeColumnMeta(eq(colAId), isNull(), eq("utf8mb4"),
								eq("utf8mb4_unicode_ci"), isNull());
				then(changeColumnMetaPort).should()
