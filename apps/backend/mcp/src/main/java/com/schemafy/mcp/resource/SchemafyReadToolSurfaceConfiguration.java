...
public class SchemafyReadToolSurfaceConfiguration {
  ...
  List<McpServerFeatures.AsyncToolSpecification> schemafyToolSpecifications(
      ...
      SchemafyHighRiskToolSurface highRiskToolSurface) {
    return Stream.concat(
        ...
        Stream.concat(highRiskToolSurface.specifications().stream(), Stream.of(tool("schemafy_list_project_presence",
            "List project presence",
            "List active project presence for a project.", idArgument("projectId", "Schemafy project ID."),
            List.of("projectId"), reader::projectPresenceTool))))
        .toList();
  }
