import { expect, test } from '@playwright/test';

const publicApi = '**/public/api/v1.0';
const smtpEnabled = (process.env.SMTP_ENABLED ?? 'false') !== 'false';

test.describe('Sign-up Flow', () => {
  test('인증 메일이 비활성화되면 인증 UI 없이 가입한다', async ({ page }) => {
    test.skip(smtpEnabled, 'SMTP_ENABLED=false 환경에서 실행한다.');

    let signUpBody: Record<string, unknown> | undefined;

    await page.route(`${publicApi}/users/signup`, async (route) => {
      signUpBody = route.request().postDataJSON();
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        headers: {
          Authorization: 'Bearer test-access-token',
          'Access-Control-Expose-Headers': 'Authorization',
        },
        body: JSON.stringify({
          id: 'user-1',
          email: 'test@example.com',
          name: 'Tester',
        }),
      });
    });

    await page.goto('/signup');

    await expect(page.getByLabel('Verification Code *')).not.toBeVisible();
    await expect(page.getByRole('button', { name: 'Send' })).not.toBeVisible();

    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('password');
    await page.getByRole('button', { name: 'Create Account' }).click();

    await expect(page).toHaveURL('http://localhost:3001/');
    expect(signUpBody).toEqual({
      email: 'test@example.com',
      name: 'Tester',
      password: 'password',
    });
  });

  test('인증 메일이 활성화되면 인증 UI와 토큰을 요구한다', async ({ page }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    let signUpRequested = false;

    await page.route(`${publicApi}/users/signup`, async (route) => {
      signUpRequested = true;
      await route.abort();
    });

    await page.goto('/signup');

    await expect(page.getByLabel('Verification Code *')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Send' })).toBeVisible();

    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('password');
    await page.getByRole('button', { name: 'Create Account' }).click();

    await expect(page.getByRole('alert')).toContainText(
      'Please verify your email to continue.',
    );
    expect(signUpRequested).toBe(false);
  });

  test('서버에서 인증 메일이 비활성화되면 전용 오류 메시지를 표시한다', async ({
    page,
  }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    await page.route(`${publicApi}/users/signup/email-code`, async (route) => {
      await route.fulfill({
        status: 409,
        contentType: 'application/problem+json',
        body: JSON.stringify({
          status: 409,
          reason: 'USER_AUTH_MAIL_DISABLED',
        }),
      });
    });

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByRole('button', { name: 'Send' }).click();

    await expect(
      page.getByText('Email verification is currently disabled.'),
    ).toBeVisible();
  });

  test('가입 검증 토큰이 만료되면 이메일 재인증 안내를 표시한다', async ({
    page,
  }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    await page.route(`${publicApi}/users/signup/email-code`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          email: 'test@example.com',
          expiresAt: '2099-01-01T00:00:00Z',
        }),
      });
    });
    await page.route(
      `${publicApi}/users/signup/email-code/verify`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            email: 'test@example.com',
            signupVerificationToken: 'expired-token',
            expiresAt: '2099-01-01T00:00:00Z',
          }),
        });
      },
    );
    await page.route(`${publicApi}/users/signup`, async (route) => {
      await route.fulfill({
        status: 409,
        contentType: 'application/problem+json',
        body: JSON.stringify({
          status: 409,
          reason: 'USER_SIGNUP_VERIFICATION_INVALID',
        }),
      });
    });

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByRole('button', { name: 'Send' }).click();
    await page.getByLabel('Verification Code *').fill('123456');
    await page.getByRole('button', { name: 'Verify' }).click();
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('password');
    await page.getByRole('button', { name: 'Create Account' }).click();

    await expect(
      page.getByText(
        'Your email verification is no longer valid. Please verify your email again.',
      ),
    ).toBeVisible();
  });

  test('이메일 인증이 끝나면 가입 요청에 인증 토큰을 포함한다', async ({
    page,
  }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    let signUpBody: Record<string, unknown> | undefined;

    await page.route(`${publicApi}/users/signup/email-code`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          email: 'test@example.com',
          expiresAt: '2099-01-01T00:00:00Z',
        }),
      });
    });
    await page.route(
      `${publicApi}/users/signup/email-code/verify`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            email: 'test@example.com',
            signupVerificationToken: 'verification-token',
            expiresAt: '2099-01-01T00:00:00Z',
          }),
        });
      },
    );
    await page.route(`${publicApi}/users/signup`, async (route) => {
      signUpBody = route.request().postDataJSON();
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        headers: {
          Authorization: 'Bearer test-access-token',
          'Access-Control-Expose-Headers': 'Authorization',
        },
        body: JSON.stringify({
          id: 'user-1',
          email: 'test@example.com',
          name: 'Tester',
        }),
      });
    });

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByRole('button', { name: 'Send' }).click();
    await page.getByLabel('Verification Code *').fill('123456');
    await page.getByRole('button', { name: 'Verify' }).click();
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('password');
    await page.getByRole('button', { name: 'Create Account' }).click();

    await expect(page).toHaveURL('http://localhost:3001/');
    expect(signUpBody).toEqual({
      email: 'test@example.com',
      name: 'Tester',
      password: 'password',
      signupVerificationToken: 'verification-token',
    });
  });

  test('코드 발송 응답의 이메일로 인증 코드를 검증한다', async ({ page }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    let verificationEmail: string | undefined;

    await page.route(`${publicApi}/users/signup/email-code`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          email: 'normalized@example.com',
          expiresAt: '2099-01-01T00:00:00Z',
        }),
      });
    });
    await page.route(
      `${publicApi}/users/signup/email-code/verify`,
      async (route) => {
        verificationEmail = route.request().postDataJSON().email;
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            email: 'normalized@example.com',
            signupVerificationToken: 'verification-token',
            expiresAt: '2099-01-01T00:00:00Z',
          }),
        });
      },
    );

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('Original@example.com');
    await page.getByRole('button', { name: 'Send' }).click();
    await page.getByLabel('Verification Code *').fill('123456');
    await page.getByRole('button', { name: 'Verify' }).click();

    await expect(page.getByText('Email verified.')).toBeVisible();
    expect(verificationEmail).toBe('normalized@example.com');
  });

  test('인증 후 이메일을 바꾸면 새 이메일 인증을 요구한다', async ({
    page,
  }) => {
    test.skip(!smtpEnabled, 'SMTP_ENABLED=true 환경에서 실행한다.');

    let signUpRequested = false;

    await page.route(`${publicApi}/users/signup/email-code`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          email: 'test@example.com',
          expiresAt: '2099-01-01T00:00:00Z',
        }),
      });
    });
    await page.route(
      `${publicApi}/users/signup/email-code/verify`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            email: 'test@example.com',
            signupVerificationToken: 'verification-token',
            expiresAt: '2099-01-01T00:00:00Z',
          }),
        });
      },
    );
    await page.route(`${publicApi}/users/signup`, async (route) => {
      signUpRequested = true;
      await route.abort();
    });

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByRole('button', { name: 'Send' }).click();
    await page.getByLabel('Verification Code *').fill('123456');
    await page.getByRole('button', { name: 'Verify' }).click();
    await expect(page.getByText('Email verified.')).toBeVisible();

    await page.getByLabel('Email *').fill('new@example.com');
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('password');
    await page.getByRole('button', { name: 'Create Account' }).click();

    await expect(page.getByRole('alert')).toContainText(
      'Please verify your email to continue.',
    );
    expect(signUpRequested).toBe(false);
  });

  test('비밀번호 확인이 다르면 Enter 제출을 차단한다', async ({ page }) => {
    let signUpRequested = false;

    await page.route(`${publicApi}/users/signup`, async (route) => {
      signUpRequested = true;
      await route.abort();
    });

    await page.goto('/signup');
    await page.getByLabel('Email *').fill('test@example.com');
    await page.getByLabel('Name *').fill('Tester');
    await page.getByLabel('Password *', { exact: true }).fill('password');
    await page.getByLabel('Confirm Password *').fill('different');
    await page.getByLabel('Confirm Password *').press('Enter');

    await expect(page.getByRole('alert').first()).toContainText(
      'Password does not match.',
    );
    expect(signUpRequested).toBe(false);
  });
});
