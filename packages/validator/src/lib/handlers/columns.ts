...
export const columnHandlers: ColumnHandlers = {
  createColumn: (database, schemaId, tableId, column) => {
    ...
    if (column.dataType) {
      ...
      if (!vendorValid) throw new ColumnDataTypeInvalidError(column.dataType, schema.dbVendorId);
    }
    ...
    if (columnNotUnique) throw new ColumnNameNotUniqueError(column.name, tableId);
