...
export const columnHandlers: ColumnHandlers = {
    createColumn: (database, schemaId, tableId, column) => {
        ...
        const isPrimaryKey = table.constraints.some(
            (constraint) =>
                constraint.kind === "PRIMARY_KEY" && constraint.columns.some((cc) => cc.columnId === column.id),
        );
    ...
    deleteColumn: (database, schemaId, tableId, columnId) => {
        ...
        const isPrimaryKey = table.constraints.some(
            (constraint) =>
                constraint.kind === "PRIMARY_KEY" && constraint.columns.some((cc) => cc.columnId === columnId),
        );
                          ...
                          tables: s.tables.map((t) =>
                              t.id === tableId
                                  ? {
                                        ...
                                        columns: t.columns.map((c) =>
                                            c.id === columnId
                                                ? { ...c, dataType, lengthScale: lengthScale || c.lengthScale }
                                                : c,
                                        ),
                                    }
                                  : t,
